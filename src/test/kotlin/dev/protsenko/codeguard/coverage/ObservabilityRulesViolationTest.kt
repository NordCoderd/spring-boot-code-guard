...
class ObservabilityRulesViolationTest {
    ...
    @Test
    fun `noStackTracePrintRule detects printStackTrace usage`() {
        val negativeScope =
            Konsist.scopeFromFiles(
                listOf("src/test/kotlin/fixtures/violations/observability/NoStackTracePrintNegative.kt"),
            )
        val error =
            assertFailsWith<AssertionError> {
                CoreRules.noStackTracePrintRule.verify(negativeScope)
            }
        assertEquals(
            "StackTracePrintingService prints stack traces in method(s): processData. " +
    ...
    @Test
    fun `noStackTracePrintRule passes for proper logging`() {
        val positiveScope =
            Konsist.scopeFromFiles(
                listOf("src/test/kotlin/fixtures/violations/observability/NoStackTracePrintPositive.kt"),
            )
        CoreRules.noStackTracePrintRule.verify(positiveScope)
    }

    @Test
    fun `noStackTracePrintRule does not flag method names or string literals`() {
        val positiveScope =
            Konsist.scopeFromFiles(
                listOf("src/test/kotlin/fixtures/violations/observability/NoStackTracePrintNameContainsPrintPositive.kt"),
            )
        CoreRules.noStackTracePrintRule.verify(positiveScope)
    }
}
