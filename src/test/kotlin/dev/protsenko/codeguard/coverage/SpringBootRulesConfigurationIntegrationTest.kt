...
class SpringBootRulesConfigurationIntegrationTest {
    ...
    @Test
    fun `all DSL context blocks register all 28 rules`() {
        val config =
            springBootRules {
        ...
        assertEquals(28, config.getAllRules().size)
    }
    ...
    fun `verifyWithResults() returns one result per configured rule`() {
        ...
        assertEquals(28, verifyResults.size)
    ...
    @Test
    fun `verify() prefixes each violation message with the rule suppress key`() {
        val error =
            assertFailsWith<AssertionError> {
                springBootRules {
                    scope =
        ...
        assertTrue(
            error.message!!.startsWith("CodeGuard:noFieldInjection:"),
            "Expected message to start with suppress key but got: ${error.message}",
        )
    }

    @Test
    fun `verifyWithResults() prefixes each failure message with the rule suppress key`() {
        val results =
            springBootRules {
                scope =
                    Konsist.scopeFromFiles(
        ...
        assertTrue(
            failure.message.startsWith("CodeGuard:noFieldInjection:"),
            "Expected failure message to start with suppress key but got: ${failure.message}",
        )
    }
