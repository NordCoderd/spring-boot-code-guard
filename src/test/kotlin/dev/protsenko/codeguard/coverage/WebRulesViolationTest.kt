...
class WebRulesViolationTest {
    ...
    @Test
    fun `requestValidationRule detects RequestBody without validation`() {
        val negativeScope =
            Konsist.scopeFromFiles(
                listOf("src/test/kotlin/fixtures/violations/web/responsehandling/RequestValidationNegative.kt"),
            )
        val error =
            assertFailsWith<AssertionError> {
                RequestHandlingRules.requestValidationRule.verify(negativeScope)
            }
        assertEquals(
            "Controller method UserController.createUser has @RequestBody parameter(s) without @Valid or @Validated: user. Validation should be enforced for request bodies.",
            error.message,
        )
    }

    @Test
    fun `requestValidationRule passes for RequestBody with validation`() {
        val positiveScope =
            Konsist.scopeFromFiles(
                listOf("src/test/kotlin/fixtures/violations/web/responsehandling/RequestValidationPositive.kt"),
            )
        RequestHandlingRules.requestValidationRule.verify(positiveScope)
    }

    @Test
    fun `requestValidationRule detects RequestParam without validation`() {
        val negativeScope =
            Konsist.scopeFromFiles(
                listOf("src/test/kotlin/fixtures/violations/web/responsehandling/RequestParamValidationNegative.kt"),
            )
        val error =
            assertFailsWith<AssertionError> {
                RequestHandlingRules.requestValidationRule.verify(negativeScope)
        ...
        assertEquals(
            "Controller method SearchController.search has @RequestParam parameter(s) without @Valid or @Validated: query. Validation should be enforced for request params.",
            error.message,
        )
    }

    @Test
    fun `requestValidationRule passes for RequestParam with validation`() {
        val positiveScope =
            Konsist.scopeFromFiles(
                listOf("src/test/kotlin/fixtures/violations/web/responsehandling/RequestParamValidationPositive.kt"),
            )
        RequestHandlingRules.requestValidationRule.verify(positiveScope)
    }
